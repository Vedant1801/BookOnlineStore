...
export default function Books() {
  ...
  return (
    <div className="py-16 px-6 md:px-16">
      ...
      <div className="flex flex-col gap-4 md:w-[18%] text-sm">
  {["All", "History", "Finance", "Science Fiction", "Fantasy", "Biography"].map((cat) => (
    <p
      key={cat}
      onClick={() => navigate(cat === "All" ? "/books" : `/books/${cat}`)}
      className={`
        flex items-center justify-between px-4 py-3 rounded-xl cursor-pointer transition-all duration-300
        border border-gray-200 shadow-sm hover:shadow-md
        hover:bg-[#3AAFA9] hover:text-white font-medium
        ${category === cat || (cat === "All" && !category) ? "bg-[#3AAFA9] text-white shadow-lg" : "bg-white text-gray-700"}
      `}
    >
      <span>{cat}</span>
      <span className={`transition-transform duration-300 ${category === cat ? "rotate-90" : ""}`}>
        ➔
      </span>
    </p>
  ))}
