...
export default function CartPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-10">
      ...
      {cart.length === 0 ? (
        ...
        <div className="space-y-6">
          {cart.map((item) => (
            <div
              key={item._id}
              className="bg-white p-6 rounded-2xl shadow-lg flex flex-col md:flex-row items-center md:justify-between gap-6 hover:shadow-xl transition-all duration-300"
            >
              {/* Book Info */}
              <div className="flex flex-col md:flex-row items-center gap-6">
                <img
                  src={item.image}
                  alt={item.title}
                  className="w-28 h-36 md:w-32 md:h-44 object-cover rounded-xl shadow-sm"
                />
                <div className="space-y-1 text-center md:text-left">
                  <h3 className="text-xl font-semibold text-gray-800">
                    {item.title}
      ...
      {/* Checkout Section */}
      {cart.length > 0 && (
        <div className="mt-12 bg-gray-50 p-6 rounded-2xl shadow-inner flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          <h3 className="text-2xl font-bold text-gray-800">
            Total: ${totalPrice.toFixed(2)}
          </h3>
          <button
            onClick={() => navigate("/check-out")}
            className="mt-4 md:mt-0 cursor-pointer px-6 py-3 text-white bg-blue-600 rounded-xl shadow-md hover:bg-blue-700 hover:shadow-lg transition-all duration-300"
          >
            Proceed to Checkout
          </button>
        </div>
