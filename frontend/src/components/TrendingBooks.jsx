    ...
    <div className="py-16 px-6 md:px-16 bg-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {trendingBooks.map((book) => (
          <div
            key={book._id}
            className="group bg-white p-4 rounded-xl shadow-lg hover:shadow-[0_0_15px_#3AAFA9] transition-all duration-200 flex flex-col items-center text-center transform hover:-translate-y-2 hover:scale-105"
          >
            <div className="overflow-hidden rounded-lg">
              <img
                src={book.image}
                alt={book.title}
                onClick={() => navigate(`/book/${book._id}`)}
                className="w-40 h-56 object-cover cursor-pointer rounded-lg transition-transform duration-500 group-hover:scale-110"
              />
            </div>
            <h3 className="mt-4 text-lg font-semibold text-gray-800">{book.title}</h3>
            <p className="text-gray-500">{book.category}</p>
            ...
            <div className="mt-3 flex items-center gap-2">
              <span className="text-lg font-bold text-green-600">${book.new_price}</span>
              <span className="text-gray-400 line-through">${book.old_price}</span>
            </div>

            <button
              onClick={() => handleAddToCart(book)}
              className="mt-3 px-6 py-2 cursor-pointer bg-[#3AAFA9] text-white rounded-lg   transition-all duration-300 transform hover:scale-105"
            >
              Add to cart
            </button>
          </div>
