...
const registerUser = async (req, res) => {
    try {
        ...
        //  const dicebearAvatar = `https://api.dicebear.com/9.x/adventurer/svg?seed=${encodeURIComponent(name)}`;

        const userData = {
            name, email, phone, password: hashedPassword // ✅ add avatar here
        }
...
const updateProfile = async (req, res) => {
    try {
        ...
        const updatedUser = await UserModel.findByIdAndUpdate(
            userId,
            { name, phone, address, dob, gender, avatar: dicebearAvatar,  ... (imageURL && { image: imageURL }) },
            { new: true }
        );
...
const getAllUuser = async (req, res) => {
  try {
    ...
    res.json({ success: true, users }); // <-- key changed to "users"
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
